package Program;
/**
 * Class LOD provides an implementation of the Instruction class that 
 * loads an argument from the data of the program into the accumulator 
 * of the processor, using either direct or indirect addressing.
 * @author CS 140
 *
 */
public class LOD extends Instruction {
	 ...
	 * stored at this new location in the data of the program becomes 
	 * the contents the accumulator. 
	...
	public void execute(Processor processor, 
		...
		if (program.getIndirect(processor.getProgramCounter())) {
			processor.setAccumulator(program.getMemory(program.getMemory(arg)));
		} else {
			processor.setAccumulator(program.getMemory(arg));
		}
